...
발표에 앞서 저를 비롯한 많은 문학, 영화사 연구자들에게 코딩 과정에서 사용되는 용어가 낯설 듯하여, 필요한 몇 가지 개념을 정리하고 가고자 한다.   
...
`API`란 응용 프로그램(application) 소프트웨어를 구측하고 정의하기 위한 프로토콜 세트로, 운영 체제(OS)와 소프트웨어 사이, 다바이스와 서버 사이, 혹은 웹 서비스 플랫폼(웹 서버)들 사이에서 이루어지는 커뮤니케이션을 위한 규약으로 활용된다.  
...
[영화관입장권통합전산망(kobis) 오픈API](http://www.kobis.or.kr/kobisopenapi/homepg/main/main.do) (이하 kobis api)를 활용하기 위해서는 해당 웹페이지에 접속하여 오픈 API가 어떤 방식으로 서비스되는지를 살펴보는 것이 중요하다. 

제공 서비스에는 `일별 박스오피스`, `주간/주말 박스오피스`, `공통코드 조회`, `영화목록`, `영화상세정보`, `영화사목록`, `영화사 상세정보`, `영화인목록`, `영화인 상세정보`가 있다. 

발표자에게 필요한 정보는 특정 년도에서 제작, 상영되었던 영화 목록이 필요한 만큼, `영화 목록` 서비스를 선택하였다. 영화목록 조회 API 서비스는 `REST` 방식과 `SOAP` 방식을 모두 지원하며, 기본 요청 URL은 `http://www.kobis.or.kr/kobisopenapi/webservice/rest/movie/searchMovieList.xml`으로 나와 있는데, `REST` 방식으로 호출하기 위해서는 마지막 확장자를 `.xml`에서 `.json`으로 바꿔주어야 한다. 
그리고 원하는 data를 서버로부터 받기 위해서는 요청 인터페이스 정보를 활용하여 요청 parameter를 입력해야 한다. 그리고 GET 방식으로 이를 호출하면 그에 맞는 응답이 이루어진다. 응답 구조는 총 16개의 값으로 구성되어 있는데, 실행을 해본 결과, 이 값들이 언제나 모두 응답되는 것은 아니고, 호출한 내용에 따라 그 응답 값이 정해지는 듯하다.  
